...
7. **G1-B1 — kappa/PsiSharp/QW source correspondence: NEXT.** The direct Section-4 finite source formula is now theorem-locked to `cutoffFreeMatrix`; the later printed (4.11)/(4.14) rewrite is non-load-bearing. Formalize or theorem-lock the source statement `QW(kappa f,kappa g)=PsiSharp(q(f,g))` with `L=2*log(lambda)`, specialized first to the existing finite Fourier sector. Only this step earns the phrase "actual localized Weil-form restriction".
8. **G2/G3 source-port / finite-to-infinite layer: OPEN.** After the actual `QW_lambda` finite restriction is theorem-locked through G1-B1, port the exact form-core/Rayleigh-bottom theorem needed by the fixed-aperture route rather than re-proving a larger spectral theory from scratch.
9. Revisit prolate/Jacobi structure only after the source restriction topology is theorem-locked. The old small-commutator-to-eigenvector route remains falsified by spectral-gap collapse.
