...
6. **G1-B0 — Section-4 source-normalization firewall: REACHED IN PR #71.** Lean independently names the raw equation-(4.4) diagonal/full matrix and the equation-(4.11)-left-hand convention. It proves the pointwise equation-(4.11) decomposition and proves that the algebraically forced correction is `exp(x/2) * cCorrectionIntegrand`. It also proves that the equation-(4.11)-left-hand gamma/arch/full matrix is exactly the independently audited `cutoffFreeMatrix`. The raw equation-(4.4) matrix is deliberately **not** identified with `cutoffFreeMatrix`; the printed integrated correction is exposed as the open proposition `SourceEq411CorrectionIdentity`.
7. **G1-B1 — source consistency / intended QW convention: NEXT.** Resolve which Section-4 expression is authoritative for the actual source `QW_lambda` finite restriction. Inspect the manuscript/TeX and executable convention, derive the diagonal directly from equation (4.4), and determine whether the printed `c(L)` integrand, equation (4.11), or equation (4.14) carries the normalization typo. Only after this seam is settled should the project formalize the kappa/PsiSharp source correspondence.
8. **G1-B2 — kappa/PsiSharp correspondence: OPEN.** The target remains `QW(kappa f,kappa g)=PsiSharp(q(f,g))` with `L=2*log(lambda)`, specialized to the already-proved finite Fourier sector. Only this step, after G1-B1, earns the phrase "actual localized Weil-form restriction".
9. **G2/G3 source-port / finite-to-infinite layer: OPEN.** After the actual QW finite restriction is theorem-locked, port the exact form-core/Rayleigh-bottom theorem needed by the fixed-aperture route rather than re-proving a larger spectral theory from scratch.
