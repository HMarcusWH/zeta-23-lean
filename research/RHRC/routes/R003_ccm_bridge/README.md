...
6. **G1-B — source correspondence / kappa map: NEXT.** Formalize or otherwise theorem-lock the source statement `QW(kappa f,kappa g)=PsiSharp(q(f,g))`, including `L=2*log(lambda)`, and prove that the G1-A additive functional is the same source form. The source proposition applies to smooth functions on `[0,L]`; the finite Fourier combinations underlying `localizedFiniteVector` are smooth before zero extension. Only this step earns the phrase "actual localized Weil-form restriction".
