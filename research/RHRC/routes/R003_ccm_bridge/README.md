...
Status: **FINITE CLOSURE REACHED / D0 SETTLED / G0-A + G0-B REACHED / G1 SOURCE-RESTRICTION FIREWALL NEXT**. RH remains **OPEN**.
...
5. **G1-A — additive source-functional restriction firewall: NEXT.** Compose the #69 half-correlation identity with the existing complex deterministic RHS theorem and `dictionaryMatrix = cutoffFreeMatrix`. This should prove the exact finite matrix for the repository additive Weil-RHS pullback while explicitly **not** defining that pullback to be the external CCM `QW_lambda`.
6. **G1-B — source correspondence / kappa map: OPEN.** Formalize or otherwise theorem-lock the source statement `QW(kappa f,kappa g)=PsiSharp(q(f,g))`, including `L=2*log(lambda)`, and prove that the G1-A additive functional is the same source form. Only this step earns the phrase "actual localized Weil-form restriction".
7. **G2/G3 source-port / finite-to-infinite layer: OPEN.** After G1-B, port the exact form-core/Rayleigh-bottom theorem needed by the fixed-aperture route rather than re-proving a larger spectral theory from scratch.
8. Revisit prolate/Jacobi structure only after the source restriction topology is theorem-locked. The old small-commutator-to-eigenvector route remains falsified by spectral-gap collapse.
