...
## Post-#247 canonical route

The active theorem-backed reduction is:

    off-line zero
     -> arbitrarily-large retained whole-cell state
     -> globally aligned parity bottom
     -> dependent even/odd/tie branch package
     -> true residual-state ground eigenmode
     -> exact #246 prime-remainder failure on the same trial
     -> GlobalBottomArithmeticResidual
     -> OPEN unconditional arithmetic/spectral exclusion

The next research programme deliberately leaves the residual-state-only world. Glasses v2 studies the unconditional ground spectrum/eigenspace across all L,N, relates full/boundary-flat/parity carriers, and then attacks a first zero crossing with fixed-cell spectral calculus plus exact prime-power seams.

This is a change of **research coordinates**, not a claim that multiple equivalent views create new information.


## Post-#237 R003 route override

Merged #237 closes the retained real completed-source corridor. On the selected-even / odd-good state, the current theorem-backed variables are real `s,m,c,f,B,q` with `B>0`, `q>0`, `c>=0`, `B<=s*f`, `(q-f)^2<=Rsharp`, `B+s^2*c<=s*m`, and `D<=s*(f-q)`.

Composing existing #231/#227 identities gives the DERIVED retained reduction `Gamma ∈ ℝ`, `f=q*Gamma`, `alpha ∈ ℝ`, `s*Gamma>0`, and `q^2*(1-Gamma)^2<=Rsharp`. The next theorem target is generic safe-negative-shift conjugation/real transfer geometry. Gamma sign, simultaneous odd-bad exclusion, odd-selected closure, parity-complete exclusion and RH remain OPEN.
...
The active R003 theorem-extraction question is **CROSS_PARITY_CORRECTION_FUNCTIONAL_RIESZ_REPRESENTATION**: export the generic mixed-resolvent formula for `oddSafeSecularCorrectionFunctional` that is already implicit in the safe Gamma proof, then derive the exact alpha formula as a corollary. PR #226 already proves the predecessor correction relation and PR #227 already formalizes the one-coefficient transfer collapse. Pair D2 retained negative-root secular completion remains downstream; any sign restriction must use canonical information about the mixed pairing rather than generic resolvent positivity alone.
...
## Post-#215 retained-root research routing
