#!/usr/bin/env python3
"""Freeze the post-#192 Q14 parity-trajectory schedule."""
...
def parity_trajectory_schedule(fixture: dict) -> dict:
    ...
    if primary_count != int(fixture["expected_primary_box_count"]):
        raise AssertionError("post-#192 primary box count drift")
    if control_count != int(fixture["expected_control_box_count"]):
        raise AssertionError("post-#192 control box count drift")
    hull = primary_hull(boxes)
    if int(hull["Q"]) != int(fixture["selected_target"]["primary_Q"]):
        raise AssertionError("post-#192 primary hull Q drift")
    return {
        "schema_version": "POST192_FB05_Q14_PARITY_TRAJECTORY_RIGIDITY_SCHEDULE_v1",
        "status": "PASS",
...
def main() -> int:
    ...
    ap.add_argument(
        "--output",
        type=Path,
        default=Path("/tmp/POST192_FB05_Q14_PARITY_TRAJECTORY_SCHEDULE.json"),
    )
    args = ap.parse_args()
    fixture = json.loads(args.input.read_text(encoding="utf-8"))
    out = parity_trajectory_schedule(fixture)
    ...
    print(json.dumps({
        ...
        "deterministic_refinement": out["policy"]["deterministic_dyadic_refinement_inside_frozen_hull_permitted"],
        "target_sign_import_permitted": out["policy"]["target_sign_import_permitted"]
    }, indent=2))
    return 0
