...
def first_zeros(n: int, cache: Path) -> np.ndarray:
    ...
    cache.parent.mkdir(parents=True, exist_ok=True); np.save(cache,arr); return arr
...
def main():
    p=argparse.ArgumentParser(); p.add_argument('--zeros',type=int,default=200); p.add_argument('--grid',type=int,default=5400); p.add_argument('--aperture-max',type=float,default=300); p.add_argument('--output',type=Path,default=OUTDIR/'r001_cached.json'); args=p.parse_args()
    t=time.time(); cache=OUTDIR/f'zeta_zeros_{args.zeros}.npy'; gs=first_zeros(args.zeros,cache); xmax=min(float(gs[-1])-5,390.0); xi=np.linspace(10,xmax,args.grid); gamma=float(gs[min(59,len(gs)-1)])
