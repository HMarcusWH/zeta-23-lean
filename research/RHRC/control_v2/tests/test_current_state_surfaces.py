...
def current_block(path: Path) -> str:
    ...
    # The current state must be encountered before a reader can enter deep history.
...
class CurrentStateSurfaceTests(unittest.TestCase):
    ...
    def test_historical_fixtures_are_not_rewritten_by_current_surface_rule(self):
        fixture = json.loads(
            (RHRC / "routes" / "R003_ccm_bridge" / "fixtures" / "post214_fb05_kernel_dual_geometry_v1.json").read_text(encoding="utf-8")
        )
