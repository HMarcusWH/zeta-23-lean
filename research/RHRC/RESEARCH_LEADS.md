...
2. **Codimension-three carrier bridge** — compare full finite Fourier, boundary-flat, and parity-compressed spectra by min-max/interlacing.
3. **N-flow bottom monotonicity** — theoremize the consequence of exact centered nesting and quadratic-form preservation.
4. **Multiplicity-safe fixed-cell L dynamics** — derivative formulas only where justified; one-sided/min-max control at degeneracy.
5. **Exact prime-power threshold update** — same operator, same normalization, real von-Mangoldt seam.
6. **Threshold-local first-crossing barrier** — positive base + fixed-cell evolution + arithmetic seam.
...
## Post-#237 lead override — real transfer geometry

**PROVED:** #237 makes the retained completed source real and proves the scalar corridor.

**DERIVED / NOT YET SEPARATELY FORMALIZED:** composing #231 and #237 gives `star Gamma * q = f` with `q > 0` and real `f`; hence `Gamma` is real and `f=q*Gamma`. The #227 affine law then makes retained `alpha` real. The #237 budget gives `s*Gamma > 0`; the sharp disk becomes `q^2*(1-Gamma)^2 <= Rsharp`.

**LEAD:** push #236 conjugation upstream to the shifted predecessor resolvent. If successful, phase freedom disappears for the whole safe negative-shift transfer system, leaving sign/orientation as the genuine obstruction.

**FALLBACK:** Pair B remains the strongest explicitly maintained independent family. High-order Riesz may supply independent arithmetic on the now-real state. **RH remains OPEN.**
...
## Historical post-#227 lead override
...
```text
#176 fixed-unit value representation
#178 complete physical-L derivative backend
#180 exact-center derivative basin
#182 generic real Schur contact calculus
#184 Hermitian/log-cover production calculus
#186 broad domination falsifier
#188 frozen selector audit
#189 individual semantic-independence audit
...
### Quarantined lead — contact locus
