...
**DEAD AS AN INDEPENDENT SUB-RH ROUTE.** If a proposed universal law on GlobalBottomResidualState is violated by every residual state, then together with #247's residual-exclusion equivalence it is RH-equivalent in strength. Such a law may still be a valid reformulation, but not independent progress.

### DR-035 — nonzero holonomy from a cycle of exact Lean equalities
**DEAD.** A cycle made only of exact equalities between the same mathematical object must close. A nonzero discrepancy diagnoses mismatched definitions, truncation/numerics or an implementation error, not an RH obstruction.

### DR-036 — ordinary Hilbert-space Gram PSD as the missing sign
**DEAD.** Gram matrices of actual Hilbert-space vectors are PSD independently of RH. Indefinite Weil-form positivity simply returns to the RH-equivalent positivity criterion.

### Still quarantined, not revived
- **DR-021:** global aperture Loewner monotonicity.
- **DR-022:** global minimizing-trial Schur monotonicity.

The **threshold-local first-crossing route remains alive** because it permits mixed derivative signs inside a cell and uses exact arithmetic seams/contact information.

**Relation-zeta:** PARKED, not declared dead. It may be revisited only if a canonical graph/dynamics arises from the mathematics itself rather than from the accidental graph of written lemmas.


## Post-#237 dead-route refinement

The old warning remains valid: positivity of a positive/self-adjoint resolvent does **not** make an arbitrary mixed pairing real or sign-controlled.

The changed classification is:
```text
mixed-pairing reality from positivity alone
    = STILL INVALID

mixed-pairing reality from canonical conjugation symmetry
    = RESURRECTED / OPEN GENERIC THEOREM ROUTE

universal Gamma positivity from reality alone
    = STILL INVALID / COUNTERMODEL FIREWALL SURVIVES
```

DR-033 also remains valid: Hermitianity alone does not justify choosing the canonical eigenvector real. #236 used conjugation plus canonical normalization; the next route seeks the stronger root-independent shifted-resolvent version of that mechanism.
...
## Historical dead-route pointer after #227

The base entries below preserve reusable failure reasons at the time they were recorded. The newest dead-route update is `DEAD_ROUTES_POST_215_DELTA.md`: DR-029 and DR-030 kill only universal full-carrier complete-source/M4 sign and proportionality. **Pair D2 retained negative-root secular completion is not dead, but it is downstream relative to the current correction-functional Riesz-representation target.**
...
**Escape requirement:** a higher-level exact correlated identity, a sharp source-specific remainder with new theorem content, or rigorously quantified interval-certified bounds resolving the observed cancellation scale. Exact paired-channel cancellation alone has now been tested in the theorem-motivated pole-prime form and remained dependency-unresolved.
