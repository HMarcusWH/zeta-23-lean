...
## Post-#247 candidate-law admission gate

Before investing theorem work in a proposed closing law C, record explicit answers:

1. Is C defined on objects that exist without assuming an off-line zero or negative residual state?
2. If RH is true, does C still have nonvacuous instances?
...
## Post-#237 validation rule

The exact #237 compiler/CI object is authoritative for the real completed-source corridor. The retained Gamma/alpha statements documented in the living SSOT are **DERIVED**, not separately theorem-promoted. Future #239 work must pass the ordinary Lean, no-placeholder, RHRC Python/control, and attached regression gates before any generic safe-shift reality statement is upgraded to PROVED.

A green research replay preserves its recorded disposition; it does not promote the derived Gamma identities or close OBS-059I. **RH remains OPEN.**
...
## Post-#227 exact-object promotion record

PR #227 is theorem authority because the exact validated final head `b8d29733167a95e16f5721eddfced6b650a3b641` passed the PR merge gate on tree `1dd1cdefcf4f4f7929b310697fdd1615a861ca1a`; the real merged commit `7aace87a5644f837e2c8b64bdcf5e66b2dc0b020` has the same tree and parents.

Passed gates include:
- `lake build Zeta23.CCM`;
- `lake build Zeta23.ExceptionalZero`;
- RHRC claim/regression suite;
- Control-v2 real-history smoke;
- forbidden `sorry` / project-axiom scan.

The headline #227 declarations print only `propext`, `Classical.choice`, and `Quot.sound`. No `sorryAx` remains.

Promoted theorem family: `CROSS_PARITY_ONE_COEFFICIENT_TRANSFER_COLLAPSE`.
Prerequisite theorem provenance: PR #226 `oddCubicGeneratorPredecessorPart_eq_neg_kappa_smul`.
Current next target: `CROSS_PARITY_CORRECTION_FUNCTIONAL_RIESZ_REPRESENTATION`.

Not promoted: alpha reality/sign, simultaneous odd-bad exclusion, selected-even WLOG, odd-selected closure, negative-root exclusion, the terminal Mathlib RH seam, or RH.


## Historical post-#224 exact-object promotion record
