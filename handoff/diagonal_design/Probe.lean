...
/-!
Scratch source for Phase F.  This file is deliberately outside the production tree.
It has not been compiler-checked: the shared compiler slot is currently reserved for
Phase D.  The first section isolates the exact quarter-digamma calculation; the second
isolates the two real improper integrals used to identify the finite-part constant.
-/
...
/-! ## From the generic density bridge to the physical diagonal functional -/

private theorem dictionaryBasisTest_diag_zero
    {L : ℝ} (hL : 0 < L) (n : ℤ) :
    dictionaryBasisTest n n L 0 = 1 := by
  simp [dictionaryBasisTest, kernel_zero hL.le]

private theorem continuous_dictionaryBasisTest_diag
    {L : ℝ} (hL : 0 < L) (n : ℤ) :
    Continuous (dictionaryBasisTest n n L) := by
  unfold dictionaryBasisTest
  exact continuous_const.mul (kernel_continuous hL n n)

private theorem integrable_dictionaryBasisTest_diag
    {L : ℝ} (hL : 0 < L) (n : ℤ) :
...
theorem integrableOn_dictionaryBasisTest_diag_sub_exp_mul_archDensity_Ioi
  ...
  have hk := continuous_dictionaryBasisTest_diag hL n
  have hki := integrable_dictionaryBasisTest_diag hL n
  ...
  filter_upwards with x
  rw [dictionaryBasisTest_diag_zero hL n]
  push_cast
  ring
...
theorem dictionaryArchRHS_basis_diag_eq_physical
  ...
  have hk : Continuous k := by
    simpa [k] using continuous_dictionaryBasisTest_diag hL n
  have hki : Integrable k := by
    simpa [k] using integrable_dictionaryBasisTest_diag hL n
  ...
  have hk0 : k 0 = 1 := by
    simpa [k] using dictionaryBasisTest_diag_zero hL n
...
/-! ## Exact `gammaL` / `betaL` diagonal algebra -/
...
/-- Phase F endpoint: the exact diagonal literature-channel archimedean entry,
including the full identity correction. -/
