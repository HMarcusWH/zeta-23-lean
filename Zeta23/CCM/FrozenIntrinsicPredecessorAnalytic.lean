...
of the identity on Euclidean space.  The proof deliberately crosses the
`toLpLin` boundary and then uses the exact matrix-vector scalar and identity
laws instead of asking simplification to discover them. -/
...
/-- After the scalar identity has acted on an ambient vector from the parity
subspace, the orthogonal projection is exactly that scalar multiple of the
original subtype vector. -/
private theorem orthogonalProjectionOnto_toLpLin_smul_one_apply
    (a : ℂ) (p : ReversalParity) (N : ℕ)
    (x : euclideanParityBoundaryFlatSubspace p N) :
    (euclideanParityBoundaryFlatSubspace p N).orthogonalProjectionOnto
        (((Matrix.toLpLin 2 2)
            (a • (1 : Matrix (Fin (2 * N + 1)) (Fin (2 * N + 1)) ℂ)))
          (x : EuclideanSpace ℂ (Fin (2 * N + 1)))) =
      a • x := by
  rw [toLpLin_smul_one_apply]
  rw [map_smul, Submodule.orthogonalProjectionOnto_mem_subspace_eq_self]
...
theorem frozenParityCompressedCanonical_eq_neg_log_id_add_remainder
  ...
  calc
    (euclideanParityBoundaryFlatSubspace p N).orthogonalProjectionOnto
      ...
      (euclideanParityBoundaryFlatSubspace p N).orthogonalProjectionOnto
          ((((-(Real.log L : ℂ)) •
              (1 : Matrix (Fin (2 * N + 1)) (Fin (2 * N + 1)) ℂ)).toEuclideanLin)
            (x : EuclideanSpace ℂ (Fin (2 * N + 1)))) +
        (euclideanParityBoundaryFlatSubspace p N).orthogonalProjectionOnto
          ((frozenCanonicalSourceRemainderReal Q L N).toEuclideanLin
    ...
    _ = (-(Real.log L : ℂ)) • x +
          frozenParityCompressedRemainder Q p L N x := by
        rw [orthogonalProjectionOnto_toLpLin_smul_one_apply]
        rfl
    _ = ((-(Real.log L : ℂ)) •
          (LinearMap.id :
            euclideanParityBoundaryFlatSubspace p N →ₗ[ℂ]
