...
The exact centered-index operator D maps the even boundary-flat sector
linearly and bijectively to the odd boundary-flat sector.  PR #102 proved that
the ambient canonical operator commutes with D on every even boundary-flat
vector, while PR #109 identified the even compression residual as lying in
span{1,d^2} and the odd normal channel as span{d}.

This module composes those facts.  After odd orthogonal compression, the d
piece is killed, so the failure of compressed D-intertwining is confined to
the single explicit line generated by the odd projection of d^3.
...
/-- The Euclidean D-restriction is injective, by exact transport of the raw
injectivity theorem. -/
...
theorem euclideanEvenToOddIndexLinearMap_surjective
  ...
  intro y
  let yr : oddBoundaryFlatSubspace N :=
    ⟨(EuclideanSpace.equiv (Fin (2 * N + 1)) ℂ)
        (y : EuclideanSpace ℂ (Fin (2 * N + 1))),
      (mem_euclideanOddBoundaryFlatSubspace_iff N _).mp y.property⟩
  obtain ⟨xr, hxr⟩ := evenToOddIndexLinearMap_surjective N yr
...
def euclideanEvenOddBoundaryFlatLinearEquiv
  ...
  LinearEquiv.ofBijective
    ...
    ⟨euclideanEvenToOddIndexLinearMap_injective N,
      euclideanEvenToOddIndexLinearMap_surjective N⟩
...
def parityCompressionResidual
  ...
  (canonicalSourceMatrix L N).toEuclideanLin
    ...
    (parityCompressedCanonical p L N v :
      EuclideanSpace ℂ (Fin (2 * N + 1)))

theorem parityCompressionResidual_mem_orthogonal
    (p : ReversalParity) (L : ℝ) (N : ℕ)
    (v : euclideanParityBoundaryFlatSubspace p N) :
    parityCompressionResidual p L N v ∈
  ...
  simpa [parityCompressionResidual, parityCompressedCanonical_apply,
    Submodule.coe_orthogonalProjectionOnto_apply, V, z] using h

theorem canonicalSourceMatrix_toEuclideanLin_mem_parityCoefficient
    (p : ReversalParity) (L : ℝ) (N : ℕ)
    {x : EuclideanSpace ℂ (Fin (2 * N + 1))}
    (hx : x ∈ euclideanParityCoefficientSubspace p N) :
    (canonicalSourceMatrix L N).toEuclideanLin x ∈
      euclideanParityCoefficientSubspace p N := by
  cases p with
  | even =>
      exact canonicalSourceMatrix_toEuclideanLin_mem_evenCoefficient L N hx
  | odd =>
      exact canonicalSourceMatrix_toEuclideanLin_mem_oddCoefficient L N hx

theorem parityCompressionResidual_mem_parityCoefficient
    (p : ReversalParity) (L : ℝ) (N : ℕ)
    (v : euclideanParityBoundaryFlatSubspace p N) :
    parityCompressionResidual p L N v ∈
      euclideanParityCoefficientSubspace p N := by
  have hvpar :
      (v : EuclideanSpace ℂ (Fin (2 * N + 1))) ∈
        euclideanParityCoefficientSubspace p N :=
    euclideanParityBoundaryFlatSubspace_le_coefficientSubspace
      p N v.property
  have hMv :=
    canonicalSourceMatrix_toEuclideanLin_mem_parityCoefficient
      p L N hvpar
  have hTv :
      (parityCompressedCanonical p L N v :
        EuclideanSpace ℂ (Fin (2 * N + 1))) ∈
          euclideanParityCoefficientSubspace p N :=
    euclideanParityBoundaryFlatSubspace_le_coefficientSubspace
      p N (parityCompressedCanonical p L N v).property
  exact (euclideanParityCoefficientSubspace p N).sub_mem hMv hTv

/-- The arbitrary even compression residual already lies in the exact
two-dimensional normal channel span{1,d^2}. -/
theorem evenCompressionResidual_mem_evenNormalSubspace
    (L : ℝ) (N : ℕ)
    (v : euclideanEvenBoundaryFlatSubspace N) :
    parityCompressionResidual .even L N v ∈ evenNormalSubspace N := by
  have h :
      parityCompressionResidual .even L N v ∈
        (euclideanEvenBoundaryFlatSubspace N)ᗮ ⊓
          euclideanEvenCoefficientSubspace N :=
    ⟨parityCompressionResidual_mem_orthogonal .even L N v,
      parityCompressionResidual_mem_parityCoefficient .even L N v⟩
...
/-- Euclidean form of the exact PR #102 even commutator collapse:
M(Du)=D(Mu). -/
...
theorem finrank_range_conjugatedParityCompressionDefect_le_one
    ...
    (N : ℕ) :
    Module.finrank ℂ
      (LinearMap.range (conjugatedParityCompressionDefect L N)) ≤ 1 := by
  let E := euclideanEvenOddBoundaryFlatLinearEquiv N
