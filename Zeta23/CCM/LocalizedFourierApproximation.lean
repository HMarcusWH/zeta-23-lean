...
theorem addCircleFourierPolynomial_coe_apply
    {L : ℝ} (c : ℤ →₀ ℂ) (x : ℝ) :
    addCircleFourierPolynomial (L := L) c (x : AddCircle L) =
      c.sum fun n a =>
        a * Complex.exp
...
theorem exists_addCircleFourierPolynomial_norm_sub_lt
    ...
    {δ : ℝ} (hδ : 0 < δ) :
    ∃ c : ℤ →₀ ℂ,
      ‖addCircleFourierPolynomial (L := L) c - F‖ < δ := by
  have hmem :
  ...
  refine ⟨c, ?_⟩
  have hpoly : addCircleFourierPolynomial (L := L) c = r := by
...
theorem fourierCoeff_addCircleFourierPolynomial
    ...
    (c : ℤ →₀ ℂ) :
    fourierCoeff (T := L)
      (addCircleFourierPolynomial (L := L) c) = c := by
      ...
      calc
        fourierCoeff
            ...
            fourierCoeff
              ...
              fourierCoeff
                (⇑(addCircleFourierPolynomial (L := L) d)) := by
                  simpa using fourierCoeff.add hci hdi
        _ = ⇑c + ⇑d := by rw [hc, hd]
        _ = ⇑(c + d) := rfl
  | single n a =>
      ...
      ext m
      change fourierCoeff
          (fun x : AddCircle L => a * fourier (T := L) n x) m =
        (Finsupp.single n a) m
...
theorem exists_zeroModeFree_addCircleFourierPolynomial_norm_sub_lt
    ...
    ∃ c : ℤ →₀ ℂ,
      c 0 = 0 ∧
      ‖addCircleFourierPolynomial (L := L) c - F‖ < 2 * δ := by
  obtain ⟨d, hd⟩ :=
    exists_addCircleFourierPolynomial_norm_sub_lt (L := L) F hδ
  let c : ℤ →₀ ℂ := d.erase 0
  have hc0 : c 0 = 0 := by
    simp [c]
  have hcoeff0 :
      fourierCoeff
          (addCircleFourierPolynomial (L := L) d - F) 0 = d 0 := by
    rw [fourierCoeff_sub, fourierCoeff_addCircleFourierPolynomial (L := L)]
    change d 0 - fourierCoeff F 0 = d 0
    rw [hF0, sub_zero]
  have hd0le : ‖d 0‖ ≤ ‖addCircleFourierPolynomial (L := L) d - F‖ := by
    have h :=
      norm_fourierCoeff_le_norm
        (L := L) (addCircleFourierPolynomial (L := L) d - F) 0
    rwa [hcoeff0] at h
  have hd0lt : ‖d 0‖ < δ :=
  ...
  calc
    ‖(addCircleFourierPolynomial (L := L) d -
        ...
        = ‖(addCircleFourierPolynomial (L := L) d - F) -
            ...
            congr 1
            abel
    _ ≤ ‖addCircleFourierPolynomial (L := L) d - F‖ +
          ‖d 0 • fourier (T := L) 0‖ :=
      norm_sub_le _ _
    _ = ‖addCircleFourierPolynomial (L := L) d - F‖ + ‖d 0‖ := by
