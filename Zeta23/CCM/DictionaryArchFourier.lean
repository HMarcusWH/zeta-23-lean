...
private theorem integrable_of_continuous_uniform_sq
  ...
  have hmajor : ∀ x : ℝ, ‖f x‖ ≤ C * (1 + x ^ 2)⁻¹ := by
    ...
    rw [mul_inv_eq_div, le_div_iff₀ hden]
    by_cases hx : x ^ 2 ≤ 1
    · have hu := hunif x
      dsimp [C]
      nlinarith
...
private theorem continuous_paperFT_real_of_integrable
    ...
    Continuous (fun r : ℝ => Zeta23.paperFT k r) := by
  have hF : Continuous (𝓕 k) :=
    VectorFourier.fourierIntegral_continuous Real.continuous_fourierChar
      (innerSL ℝ).continuous₂ hk
  have heq :
      (fun r : ℝ => Zeta23.paperFT k r) =
...
private theorem integrable_fourier_of_integrable_paperFT
    {k : ℝ → ℂ}
    (hk : Integrable (fun r : ℝ => Zeta23.paperFT k r)) :
    Integrable (𝓕 k) := by
  have hc : (-(2 * Real.pi) : ℝ) ≠ 0 := by positivity
  have hcomp := hk.comp_mul_left' hc
  have heq :
      (fun w : ℝ => Zeta23.paperFT k (-(2 * Real.pi) * w)) = 𝓕 k := by
    funext w
    rw [Zeta23.fourier_eq_paperFT]
    congr 1
    ring
  rwa [heq] at hcomp
...
theorem integrable_paperFT_dictionaryTent
    {L : ℝ} (hL : 0 < L) :
    Integrable (fun r : ℝ => Zeta23.paperFT (dictionaryTent L) r) := by
  ...
  have hunif : ∀ r : ℝ,
      ‖Zeta23.paperFT (dictionaryTent L) r‖ ≤ A := by
    intro r
  ...
  have hB : 0 ≤ B := by
    ...
    positivity
  have hsq : ∀ r : ℝ,
      ‖Zeta23.paperFT (dictionaryTent L) r‖ * r ^ 2 ≤ B := by
    intro r
...
theorem integrable_fourier_dictionaryBasisTest_diag
  ...
  have ht : Integrable (fun r : ℝ => Zeta23.paperFT (dictionaryTent L) r) :=
    integrable_paperFT_dictionaryTent hL
  let a : ℝ := dictionaryFrequency n L
  have hp : Integrable
      (fun r : ℝ => Zeta23.paperFT (dictionaryTent L) (r + a)) :=
    ht.comp_add_right a
  have hm : Integrable
      (fun r : ℝ => Zeta23.paperFT (dictionaryTent L) (r - a)) := by
    simpa [sub_eq_add_neg] using ht.comp_add_right (-a)
  have hsum : Integrable
      (fun r : ℝ =>
        Zeta23.paperFT (dictionaryTent L) (r + a) +
          Zeta23.paperFT (dictionaryTent L) (r - a)) :=
    hp.add hm
  ...
  have heq :
      (fun r : ℝ => Zeta23.paperFT (dictionaryBasisTest n n L) r) =
        fun r : ℝ =>
          (1 / 2 : ℂ) *
            (Zeta23.paperFT (dictionaryTent L) (r + a) +
              Zeta23.paperFT (dictionaryTent L) (r - a)) := by
    funext r
    rw [paperFT_dictionaryBasisTest_diag hL n r]
    dsimp [a]
    ring
  have hpaper : Integrable
      (fun r : ℝ => Zeta23.paperFT (dictionaryBasisTest n n L) r) := by
    rw [heq]
    exact hhalf
